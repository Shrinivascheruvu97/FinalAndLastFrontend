...
const renderPieChart = (role) => {
  ...
  //               style={{ fontWeight: 'bold', fill: 'white' }} // Change the text color to white
  //             >
  //               {data[index].name}
  //               <tspan x={x} dy={15}>
  //                 votes: {value}
    ...
    <div>
      <PieChart width={440} height={360}>
        <Pie
          ...
          label={({ cx, cy, midAngle, innerRadius, outerRadius, value, index }) => {
            ...
            return (
              <text
                ...
                style={{
                  fontWeight: 'bold',
                  fill: activeIndex === index ? 'white' : '#8884d8',
                }} // Change the text color to white for the active slice
              >
                {data[index].name}
                <tspan x={x} dy={15}>
                  votes: {value}
          ...
          onMouseEnter={handleMouseEnter} // Add event handlers for hover effect
          onMouseLeave={handleMouseLeave}
        >
          {data.map((entry, index) => (
            <Cell
              key={entry.name}
              fill={activeIndex === index ? '#FF6384' : colors[index % colors.length]}
            />
          ))}
        </Pie>
        <Tooltip />
...
<h3 style={{ color: 'purple', marginTop: '40px', marginLeft: '10px'}}>SSS Society <br/> <h6>Transforming your lives with joy... </h6></h3><br/>
...
<Modal show={showAddModal} onHide={handleAddEvent}>
      <Modal.Header closeButton>
        ...
        <Button variant='link' onClick={handleShowRatesDialog}>
               <InfoIcon/>
              </Button>
      </Modal.Header>
      <Modal.Body>
    ...
    />
     ...
     {/* Rates Dialog */}
     <Modal show={showRatesDialog} onHide={handleCloseRatesDialog}>
        <Modal.Header closeButton>
          <Modal.Title>Rate Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>For 1 hr: 1000 rs</p>
          <p>For half day: 2500 rs</p>
          <p>For 1 day: 5000 rs</p>
        </Modal.Body>
      </Modal>
{/* ---------------------------------------------------------------------------------------------------------------------------------- */}
